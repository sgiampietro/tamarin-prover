...
maudeSig msig@MaudeSig{enableDH, enableBP, enableMSet, enableNat, enableXor, enableDHMult, enableDiff = _, stFunSyms, stRules, eqConvergent} =
  ...
  where
    allfuns = S.map NoEq stFunSyms
                ...
                `S.union` (if enableDHMult         then dhMultFunSig  else S.empty)
...
instance Semigroup MaudeSig where
    MaudeSig dh1 bp1 mset1 nat1 xor1 dhmult1 diff1 stFunSyms1 stRules1 _ _ _ _ <>
      MaudeSig dh2 bp2 mset2 nat2 xor2 dhmult2 diff2 stFunSyms2 stRules2 _ _ _ _ =
...
instance Monoid MaudeSig where
    mempty = MaudeSig False False False False False False False S.empty S.empty False S.empty S.empty S.empty
...
prettyMaudeSigExcept sig excl = P.vcat
  ...
  where
    ...
    builtIns = asum $ map (\(f, x) -> guard (f sig) *> pure x)
      ...
      , (enableXor,  "xor")
      , (enableDHMult,  "DH-multiplication")
      ]
