...
contradictions ctxt sys = F.asum
    ...
    , guard (enableDHMult msig && hasAdvKnowsBasis sys) $> KnowsBasisEl
...
hasNonNormalTerms sig se =  -- trace ("non-normal terms" ++ show (filter (not . (`runReader` hnd) . nf') (maybeNonNormalTerms hnd se)) ++ " -- " ++ show (map ((`runReader` hnd) . nf') (maybeNonNormalTerms hnd se)) ) $
